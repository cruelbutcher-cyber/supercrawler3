...
class EnhancedWebCrawler:
    ...
    def extract_urls_from_onclick(self, element, base_url):
        ...
        if element.has_attr('onclick'):
            ...
            for pattern in patterns:
                matches = re.findall(pattern, onclick_js)
                for match in matches:
                    full_url = urljoin(base_url, match)
                    onclick_urls.append(full_url)
        return list(set(onclick_urls))

    def get_matched_keywords(self, text):
        """Check which keywords are present in the given text"""
    ...
    def is_relevant_path(self, url):
        ...
        if re.search(r'/(login|logout|register|signin|signout|cart|checkout|privacy|terms)/?$', path):
            return False
        relevant_patterns = [
            r'/(post|article|blog|news|story|travel|guide|destination|affiliate)/',
            r'/search',
            r'/visit',
            r'/finder',
            r'/details',
        ...
        return any(re.search(p, path) for p in relevant_patterns)
    ...
    def looks_like_affiliate_url(self, url):
        ...
        if any(re.search(pattern, url_lower) for pattern in self.redirect_patterns):
            return True
        ...
        if any(domain in netloc for domain in self.awin_domains):
            if '87121' in url_lower:
                return True
            query_params = parse_qs(parsed_url.query)
            if 'awinaffid' in query_params or 'awinmid' in query_params:
                return True
                
        query_params = parse_qs(parsed_url.query)
        tracking_params = ['aff', 'affid', 'affiliateid', 'ref', 'refid', 'referral', 
                          'partner', 'partnerId', 'utm_source', 'awc', 'sv1']
        for param in tracking_params:
            if param in query_params:
                return True
                
        path_segments = parsed_url.path.split('/')
        if any(seg in ['affiliate', 'partner', 'track', 'redirect'] for seg in path_segments):
            return True
    ...
    def extract_redirection_url(self, html_content, url):
        ...
        if meta_refresh and meta_refresh.get('content'):
            ...
            if 'url=' in content:
                redirect_url = content.split('url=', 1)[1].split(';', 1)[0].strip()
                redirect_urls.append(urljoin(url, redirect_url))
                
        # Improved JavaScript redirect detection
        script_patterns = [
            r'(?:window\.location|document\.location|window\.open)\(?[\'"]([^\'"]+)[\'"]\)?',
            r'window\.location\.replace\([\'"](.+?)[\'"]\)',
            r'location\.href\s*=\s*[\'"](.+?)[\'"]',
            r'url:\s*[\'"](https?://.+?)[\'"]',
            r'redirectUrl\s*:\s*[\'"](https?://.+?)[\'"]',
            r'clickUrl\s*:\s*[\'"](https?://.+?)[\'"]'
        ]
        
        # Detect URLs in JSON-LD data
        for script in soup.find_all('script', {'type': 'application/ld+json'}):
            try:
                json_data = json.loads(script.string)
                for key, value in json_data.items():
                    if 'url' in key.lower() and isinstance(value, str):
                        redirect_urls.append(urljoin(url, value))
            except json.JSONDecodeError:
                pass
                
        # Find URLs in data attributes
        for element in soup.find_all(attrs={'data-url': True}):
            redirect_urls.append(urljoin(url, element['data-url']))
            
        # Find URLs in iframe src
        for iframe in soup.find_all('iframe', {'src': True}):
            redirect_urls.append(urljoin(url, iframe['src']))
            
        return list(set(redirect_urls))
    def resolve_redirects(self, url, depth=0, max_depth=7):
        if depth > max_depth:
            return url, []
        ...
        try:
            response = self.session.head(url, allow_redirects=False, timeout=15)
            if 300 <= response.status_code < 400:
                location = response.headers.get('Location', '')
                if location:
                    redirect_url = urljoin(url, location)
                    final_url, history = self.resolve_redirects(redirect_url, depth+1, max_depth)
                    self.redirect_cache[url] = (final_url, [url] + history)
                    return final_url, [url] + history
            ...
            self.logger.error(f"Redirect resolution error: {str(e)}")
            return url, []
    ...
    def check_url_for_keywords(self, url, source_url):
        ...
        for kw in self.keywords:
            if kw in unquote(parsed_url.path) or kw in unquote(parsed_url.query):
                self.add_result(
                    ...
                    content=url,
                    keywords=[kw],
                    location_type='direct_url_fragment'
                )
        final_url, history = self.resolve_redirects(url)
        all_urls = [url] + history + [final_url]
        
        for checked_url in all_urls:
            parsed_checked = urlparse(checked_url)
            query_params = parse_qs(parsed_checked.query)
            
            if 'awin1.com' in parsed_checked.netloc:
                if query_params.get('v') == ['87121'] or '87121' in parsed_checked.path:
                    self.add_result(
                        source_url=source_url,
                        matched_url=checked_url,
                        element='url',
                        attribute='href',
                        content=f"AWIN link with ID 87121: {checked_url}",
                        keywords=['87121'],
                        location_type='awin_affiliate_url'
                    )
                    
            decoded_url = unquote(checked_url)
            matched_kws = self.get_matched_keywords(decoded_url)
            if matched_kws:
                self.add_result(
                    source_url=source_url,
                    matched_url=checked_url,
    ...
    def process_url(self, url):
        ...
        # Enhanced shadow DOM and dynamic content handling
        for template in soup.find_all('template'):
            shadow_content = str(template)
            shadow_soup = BeautifulSoup(shadow_content, 'html.parser')
            for element in shadow_soup.find_all(['a', 'img', 'meta']):
                self.check_element(element, final_url)
                
        # Process linked JSON data
        for link in soup.find_all('link', {'type': 'application/json'}):
            try:
                json_url = urljoin(final_url, link['href'])
                json_response = self.session.get(json_url)
                if json_response.status_code == 200:
                    self._extract_urls_from_json(json_response.json(), final_url, [])
            except Exception:
                pass
                
        # Process microdata and structured data
        for item in soup.select('[itemtype]'):
            for prop in item.select('[itemprop]'):
                if prop.get('href'):
                    self.check_url_for_keywords(urljoin(final_url, prop['href']), final_url)
        ...
        for script in script_tags:
            if script.string:
                ...
                json_pattern = r'\{.*\}'
                if re.search(json_pattern, script.string):
                    try:
                        json_data = json.loads(script.string)
                        self._extract_urls_from_json(json_data, final_url, [])
                    except json.JSONDecodeError:
                        pass
                
                # Detect URLs in JavaScript variables
                js_urls = re.findall(r'var\s+\w+Url\s*=\s*[\'"](https?://[^\'"]+)[\'"]', script.string)
                for js_url in js_urls:
                    self.check_url_for_keywords(js_url, final_url)
        # Improved image banner detection
        for a_tag in soup.find_all('a'):
            if a_tag.find('img'):
                ...
                if img_src:
                    ...
                    self.check_url_for_keywords(img_url, final_url)
                parent_url = urljoin(final_url, a_tag.get('href', ''))
                self.check_url_for_keywords(parent_url, final_url)

        # Enhanced affiliate parameter detection
        for element in soup.find_all(['a', 'form', 'div']):
            for attr in element.attrs:
                if 'data-affiliate' in attr or 'data-track' in attr or 'data-ref' in attr:
                    attr_value = element[attr]
                    if isinstance(attr_value, str) and any(kw in attr_value for kw in self.keywords):
                        self.add_result(
                            ...
                            attribute=attr,
                            content=attr_value,
                            keywords=self.get_matched_keywords(attr_value),
